...
class Mensajes {
	...
	private static $fichero_nombre = "mensajes.txt";
	
	private static $fila_definicion = array(
		"idioma" => "errores_requerido && errores_texto",
		"fecha" => "errores_requerido && errores_fecha_mysql",
		"autor" => "errores_requerido && errores_identificador",
		"texto" => "errores_requerido && errores_texto"
		
	);
	
	/**
	 * Almacena cada línea del fichero de texto como un array con la estructura
	 * definida en $fila_definicion.
	 * 
	 * @var array 
	 */
	private static $filas = array();
	...
	private static function leer_de_fichero() {
		...
		foreach ($lineas as $numero => $linea) {
			...
			if ($numero != 0) {
				$i = 0;
				foreach (self::$fila_definicion as $key => $value) {
					self::$filas[$numero][$key] = $fila[$i]; 
					$i++;
				}
			}
		}
